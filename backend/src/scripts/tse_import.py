...
def process_import(ano, uf, municipio_nome, nr_candidato, tenant_id):
    ...
    try:
        ...
        cur = conn.cursor()
        report_progress(tenant_id, "Minerando dados do TSE...", 5)
        ...
        for file in files:
            ...
            # Em 2024, o código pode estar em SG_UE ou CD_UE ou CD_MUNICIPIO
            ...
            if city_col and num_col:
                ...
                if not cand.empty:
                    ...
                    # Garantia contra KeyError: None
                    c_id = c[cd_mun_col] if cd_mun_col else "0"
                    c_name = c[city_col] if city_col else municipio_nome.upper()
                    c_num = c[num_col] if num_col else nr_cand_str
                    
                    print(f"✅ CANDIDATO LOCALIZADO: {c['NM_CANDIDATO']} (Código Município: {c_id})")
                    ...
                    """, (tenant_id, ano, c['NM_CANDIDATO'], c_num, c['SG_PARTIDO'], c_id, c_name, c['DS_SITUACAO_CANDIDATURA']))
                    found_candidato = True
                    break

        if not found_candidato:
            report_progress(tenant_id, f"Não encontramos {nr_cand_str} em {municipio_nome}.", 0)
            return

        # 2. Locais
        report_progress(tenant_id, "Buscando Bairros...", 35)
        ...
        for file in os.listdir(tmp_dir):
            if file.lower().endswith('.csv') and 'rede_locais_votacao' in file.lower():
                ...
                c_code = find_column(df_loc.columns, ['CD', 'MUN']) or find_column(df_loc.columns, ['SG', 'UE']) or find_column(df_loc.columns, ['CD', 'UE'])
                
                if c_city and c_code:
            ...
            for file in os.listdir(tmp_dir):
                if file.lower().endswith('.csv') and 'votacao_secao' in file.lower():
                    ...
                    for chunk in chunks:
                        ...
                        c_code_v = find_column(chunk.columns, ['CD', 'MUN']) or find_column(chunk.columns, ['SG', 'UE']) or find_column(chunk.columns, ['CD', 'UE'])
                        
                        if c_city_v and c_code_v:
